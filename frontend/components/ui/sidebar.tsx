...
function Sidebar({
  ...
  /** Activa el skin visual “Figma” (fondo var(--sidebar), halos y borde redondeado). */
  figmaSkin?: boolean
}) {
  const { isMobile, state, openMobile, setOpenMobile } = useSidebar()

  const containerBase = cn(
    // padding/bordes según variante
  ...
  const innerBase = cn(
    ...
    // Skin figma: fondo, border y sombra (aunque la variable bg-sidebar sigue mandando)
    figmaSkin && "relative overflow-hidden border border-sidebar-border/40 rounded-lg shadow-lg bg-[var(--sidebar)]"
  )
  ...
  if (collapsible === "none") {
    return (
      <div
        ...
        {...props}
      >
        <div className={cn("relative h-full w-full", figmaSkin && "rounded-lg overflow-hidden")}>
          {figmaSkin && <FigmaHalos />}
          {children}
        </div>
      </div>
    )
  ...
  return (
    ...
    >
      ...
      />
      <div data-slot="sidebar-container" className={containerBase} {...props}>
        <div data-sidebar="sidebar" data-slot="sidebar-inner" className={innerBase}>
          {figmaSkin && <FigmaHalos />}
          {children}
        </div>
      </div>
...
function SidebarMenuButton({
  ...
  /** Aplica el estilo “glass card” y estados de color como en el diseño figma. */
  figma?: boolean
  ...
  const base = cn(
    ...
    figma &&
      "rounded-2xl px-4 py-4 h-auto border-0 text-white/85 hover:bg-white/20 hover:text-white data-[active=true]:bg-white/30 data-[active=true]:text-white data-[active=true]:shadow-lg data-[active=true]:border data-[active=true]:border-white/25 backdrop-blur-sm",
    className
  )
