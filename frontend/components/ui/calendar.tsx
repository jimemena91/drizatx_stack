...
function Calendar({
  ...
  components,
  ...props
}: React.ComponentProps<typeof DayPicker> & { buttonVariant?: ButtonVariant }) {
  ...
  return (
    <DayPicker
      ...
      className={cn(
        // Contenedor con tokens
        "group/calendar p-3 bg-card text-card-foreground border border-border/60 rounded-lg shadow-xs",
        // Evita duplicar fondo en Card/Popover
        String.raw`[[data-slot=card-content]_&]:bg-transparent [[data-slot=popover-content]_&]:bg-transparent border-0 shadow-none`,
        // RTL: rota íconos
      ...
      classNames={{
        ...
        dropdown_root: cn(
          "relative rounded-md border border-input has-focus:border-ring has-focus:ring-[3px] has-focus:ring-ring/50 shadow-xs",
        ...
        day: cn(
          [
            "group/day relative aspect-square w-full select-none p-0 text-center leading-none",
            // Bordes redondeados en rangos
        ...
        today: cn(
          // Hoy con anillo sutil (si no está seleccionado)
          "rounded-md ring-1 ring-border data-[selected=true]:ring-0 bg-muted text-foreground data-[selected=true]:rounded-none",
